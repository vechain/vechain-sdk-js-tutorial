...
import { type InvalidDataType } from '@vechain/sdk-errors';
...
const bloomFilter = BloomFilter.of(
    ...set.map((element) => element.bytes)
).build();
...
const key = set[0];
console.log(
    `Is "${key}" in ${Hex.of(bloomFilter.bytes)} filter made from bytes?`
);
...
console.log(
    `Is "${key}" in ${Hex.of(hexBloomFilter.bytes)} filter made from hex?`
);
...
try {
    ...
    console.error(
        `Cast arrayBloomFilter as a number throws "${invalidDataType.errorMessage}"!`
    );
}
...
if (bloomFilter.isJoinable(additionalBloomFilter)) {
    ...
    set.forEach((element) => {
        ...
        console.log(
            `${element}\t\t${isInSet}\t${isInAdditionalSet}\t\t${isInUnionSet}`
        );
    });
...
console.log(
    `Original ${Hex.of(bloomFilter.bytes)}, clone ${Hex.of(cloneFilter.bytes)}.`
);
